...
function AttackCombo(){
	...
	while(conCombo > 0){
		if(!useMecanim){
			...
			var clip = GetComponent(PlayerMecanimAnimation).animator.GetCurrentAnimationClipState(0);
...
function MagicSkill(skillID : int){
	...
	if(!useMecanim){
		//For Legacy Animation
		mainModel.animation[skillAnimation[skillID].name].layer = 16;
		mainModel.animation[skillAnimation[skillID].name].speed = skillAnimationSpeed;
...
function Aiming(){
		var ray : Ray = Maincam.camera.ViewportPointToRay (Vector3(0.5,0.5,0));
		// Do a raycast
		var hit : RaycastHit;
