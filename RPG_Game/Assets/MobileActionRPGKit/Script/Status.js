...
function OnWebbedUp(dur : float){
		...
		if(per <= chance){
			...
			if(webbedUpAnimation){// If you Assign the Animation then play it
				if(useMecanim){
					...
					mainModel.animation[webbedUpAnimation.name].layer = 25;
					mainModel.animation.Play(webbedUpAnimation.name);
				}
			}
		}else{
			return;
	...
	if(webbedUpAnimation && !useMecanim){// If you Assign the Animation then stop playing
		mainModel.animation.Stop(webbedUpAnimation.name);
	}
...
function OnStun(dur : float){
		...
		if(per <= chance){
			...
			if(stunAnimation){// If you Assign the Animation then play it
				if(useMecanim){
					...
					mainModel.animation[stunAnimation.name].layer = 25;
					mainModel.animation.Play(stunAnimation.name);
				}
			}
		}else{
			return;
	...
	if(stunAnimation && !useMecanim){// If you Assign the Animation then stop playing
		mainModel.animation.Stop(stunAnimation.name);
	}
	freeze = false; // Freeze Character Off
	stun = false;
