	...
	GetComponent(Status).mainModel = mainModel;
		...
		matk = GetComponent(Status).matk;
      	...
      	if(!useMecanim){
	      	...
	      	mainModel.animation.Play(idleAnimation.name);
	        mainModel.animation[hurtAnimation.name].layer = 10;
        }else{
...
function Flinch(dir : Vector3){
	...
	knock = transform.TransformDirection(Vector3.back);
		...
		if(!useMecanim){
        	//If using Legacy Animation
			mainModel.animation.PlayQueued(hurtAnimation.name, QueueMode.PlayNow);
			mainModel.animation.CrossFade(movingAnimation.name, 0.2f);
		}
		followState = AIState.Moving;
...
function Attack(){
	...
	/*if(attackVoice && !flinch){
		...
		if(!useMecanim){
        	//If using Legacy Animation
			mainModel.animation.Play(attackAnimation.name);
		}else{
