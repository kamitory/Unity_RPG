...
function Update () {
	...
	if(flying == 1){
		if(!useMecanim && flyDownAnimation){
        	...
        	mainModel.animation.CrossFade(flyDownAnimation.name, 0.2f);
        }else{
	...
	if(flying == 2){
		if(!useMecanim && flyUpAnimation){
        	//If using Legacy Animation
        	mainModel.animation.CrossFade(flyUpAnimation.name, 0.2f);
        }else{
...
function Landing(){
	...
	if(landingAnimation && !useMecanim){
		...
		mainModel.animation.Play(landingAnimation.name);
	}else if(useMecanim){
